...
9. Download [XPerience Recovery](https://sourceforge.net/projects/xperience-aosp/files/{{ page.device }}/{{ page.versions }}/nightly/{{ custom_recovery_codename }}). Simply download the latest recovery file, named `{{ device.recovery_partition_name }}.img`.
...
14. Now reboot into recovery to verify the installation.
    ...
    * {{ device.recovery_boot }}
    {%- endif %}
{%- assign is_xperience_recovery = true %}
{%- include snippets/recovery_logo_note.md %}
{%- if device.before_XPerience_install %}
{% capture path %}templates/device_specific/before_XPerience_install_{{ device.before_XPerience_install }}.md{% endcapture %}
{% include {{ path }} %}
{%- endif %}
15. Wipe your data partition (this is usually named "Wipe", or "Format")
{%- elsif device.format_on_upgrade < device.current_branch %}
8. If your device is running The XPerience Project version older than {{ device.format_on_upgrade | precision: 1 }}, wipe your data partition (this is usually named "Wipe", or "Format").
{%- else %}
