...
{%- assign is_xperience_recovery = device.custom_xperience_recovery %}
...
1. Download [xperience Recovery](https://sourceforge.net/projects/xperience-aosp/files/recovery/{{ device.codename }}). Simply download the latest recovery file, named `{{ device.recovery_partition_name }}.img`.
{%- else %}
{%- assign is_xperience_recovery = true %}
1. [Build]({{ device | device_link: "/build" | relative_url }}) a XPerience installation package. The recovery will be built as part of it!
{%- endif %}
...
{%- if is_xperience_recovery %}
## Installing XPerience Recovery using `fastboot`
{%- else %}
